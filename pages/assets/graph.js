(function () {
  ...
  function draw() {
    ...
    // Outer soft glow (pulsing) — 2x size
    var g1 = ctx.createRadialGradient(x0, y0, 0, x0, y0, 160 + pulse * 40);
    g1.addColorStop(0, 'rgba(124, 92, 255, ' + (0.18 + pulse * 0.12) + ')');
    g1.addColorStop(0.5, 'rgba(124, 92, 255, ' + (0.06 + pulse * 0.05) + ')');
    g1.addColorStop(1, 'rgba(124, 92, 255, 0)');
    ctx.beginPath();
    ctx.arc(x0, y0, 200, 0, Math.PI * 2);
    ctx.fillStyle = g1;
    ctx.fill();

    // Star rays (cross-shaped light beams) — 2x length
    ctx.save();
    ctx.translate(x0, y0);
    ctx.rotate(starPhase * 0.1);
    var rayLen = 80 + pulse * 30;
    for (var ri = 0; ri < 4; ri++) {
      var ra = ri * Math.PI / 2;
      var g2 = ctx.createLinearGradient(0, 0, Math.cos(ra) * rayLen, Math.sin(ra) * rayLen);
      g2.addColorStop(0, 'rgba(200, 180, 255, ' + (0.6 + pulse * 0.4) + ')');
      g2.addColorStop(1, 'rgba(124, 92, 255, 0)');
      ctx.beginPath();
      ctx.moveTo(Math.cos(ra - 0.15) * 12, Math.sin(ra - 0.15) * 12);
      ctx.lineTo(Math.cos(ra) * rayLen, Math.sin(ra) * rayLen);
      ctx.lineTo(Math.cos(ra + 0.15) * 12, Math.sin(ra + 0.15) * 12);
      ctx.closePath();
      ctx.fillStyle = g2;
      ctx.fill();
    }
    // Diagonal rays (thinner) — 2x length
    for (var di = 0; di < 4; di++) {
      var da = di * Math.PI / 2 + Math.PI / 4;
      var dLen = 50 + pulse * 20;
      var g3 = ctx.createLinearGradient(0, 0, Math.cos(da) * dLen, Math.sin(da) * dLen);
      g3.addColorStop(0, 'rgba(200, 180, 255, ' + (0.4 + pulse * 0.3) + ')');
      g3.addColorStop(1, 'rgba(124, 92, 255, 0)');
      ctx.beginPath();
      ctx.moveTo(Math.cos(da - 0.1) * 10, Math.sin(da - 0.1) * 10);
      ctx.lineTo(Math.cos(da) * dLen, Math.sin(da) * dLen);
      ctx.lineTo(Math.cos(da + 0.1) * 10, Math.sin(da + 0.1) * 10);
      ctx.closePath();
      ctx.fillStyle = g3;
      ctx.fill();
    }
    ...
    // Inner bright core — 2x size
    var g4 = ctx.createRadialGradient(x0, y0, 0, x0, y0, 36);
    g4.addColorStop(0, 'rgba(230, 220, 255, ' + (0.95 + pulse * 0.05) + ')');
    g4.addColorStop(0.4, 'rgba(155, 127, 255, 0.7)');
    g4.addColorStop(1, 'rgba(124, 92, 255, 0)');
    ctx.beginPath();
    ctx.arc(x0, y0, 36, 0, Math.PI * 2);
    ctx.fillStyle = g4;
    ctx.fill();

    // Bright center dot — 2x size
    ctx.beginPath();
